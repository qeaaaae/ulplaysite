...
@endphp
<article class="group relative flex flex-col h-full min-h-[220px] sm:min-h-[240px] md:min-h-[260px] rounded-xl overflow-hidden border border-stone-200 shadow-[0_1px_3px_0_rgba(0,0,0,0.06),0_1px_2px_-1px_rgba(0,0,0,0.06)] hover:border-sky-200 hover:shadow-[0_4px_6px_-1px_rgba(0,0,0,0.08),0_2px_4px_-2px_rgba(0,0,0,0.06)] transition-all duration-300 cursor-pointer">
    <a href="{{ route('services.show', $service) }}" class="absolute inset-0 z-20" aria-label="{{ $service->title }}"><span class="sr-only">{{ $service->title }}</span></a>
    <div class="relative flex-1 flex flex-col min-h-0">
        ...
        <div class="relative z-10 mt-auto p-4 sm:p-5 md:p-6 flex flex-col pointer-events-none">
