...
@section('content')
    <form method="POST" action="{{ $banner->id ? route('admin.banners.update', $banner) : route('admin.banners.store') }}" enctype="multipart/form-data" class="w-full space-y-4">
        @csrf
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-sm {{ $banner->id ? 'text-stone-600' : 'text-stone-500' }}">{{ $banner->id ? $banner->title : 'Заполните поля' }}</p>
                    </div>
                </div>
            </div>
        ...
        <x-admin.form-section title="Баннер" icon="heroicon-o-photo">
            ...
            <div class="mt-4 flex flex-col">
                ...
                <textarea name="description" rows="2" class="w-full px-3 py-2.5 bg-white border border-stone-300 rounded-md text-stone-900 placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-sky-500/30 focus:border-sky-500 transition-colors duration-150 resize-y" placeholder="Краткое описание баннера">{{ old('description', $banner->description) }}</textarea>
            </div>
            <div class="mt-4">
                <x-ui.file-input name="image" accept="image/*" label="Файл изображения" label-icon="heroicon-o-photo" :existing-url="$banner->id ? $banner->image : null" :lightbox-group="$banner->id ? 'admin-banner-' . $banner->id : null" />
            </div>
