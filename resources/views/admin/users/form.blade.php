...
@section('content')
    <form method="POST" action="{{ $user->id ? route('admin.users.update', $user) : route('admin.users.store') }}" class="w-full space-y-6">
        ...
        <x-admin.form-section title="Основное" icon="heroicon-o-user-circle">
        ...
        <x-admin.form-section title="Права" icon="heroicon-o-shield-check">
            <div class="flex flex-wrap gap-6 lg:col-span-2">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="email_verified" value="1" {{ old('email_verified', $user->email_verified_at) ? 'checked' : '' }} class="rounded border-stone-300 text-sky-600 accent-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-500/30">
                    <span class="text-sm text-stone-700">Email подтверждён</span>
                </label>
            </div>
        </x-admin.form-section>
    </form>
